...
function form() {
  ...
  form.addEventListener("submit", function (event) {
    event.preventDefault();
    form.appendChild(statusMessage); //     let request = new XMLHttpRequest();
...
function promise() {
  ...
  function shoot(arrow) {
    console.log('Вы сделали выстрел...'); // resolve = когда обещание выполнилось
...
function tabs() {
  ...
  ;
  hideTabContent(1); // Показать какой-то таб контент
...
function timer() {
  var deadLine = "2019-05-31"; // Конечная дата

  function getTimeRemaining(endtime) {
    var dateDifference = Date.parse(endtime) - Date.parse(new Date()),
        // Разница между датами
    seconds = Math.floor(dateDifference / 1000 % 60),
        // Секунды
    minutes = Math.floor(dateDifference / 1000 / 60 % 60),
        // Минуты
    hours = Math.floor(dateDifference / (1000 * 60 * 60)),
        // Часы
  ...
  function setClock(id, endtime) {
    ...
    function updateClock() {
      var t = getTimeRemaining(endtime); // Проверка на единицы. Если цифра одна то добавлять 0 перед ней V1
      // if ((t.seconds % 100) < 10) {
      ...
      seconds.textContent = addZero(t.seconds); // Если время вышло или введена прошедшая дата

      if (t.total <= 0) {
        var timeAction = document.querySelector(".timer-action");
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  var calc = __webpack_require__(/*! ./parts/calc */ "./js/parts/calc.js"),
      ...
      promise = __webpack_require__(/*! ./parts/promise */ "./js/parts/promise.js");
